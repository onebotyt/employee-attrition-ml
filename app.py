...
# ════════════════════════════════════════════════════════════════════════════
#  PAGE 9: HOW TO RUN & HOST
# ════════════════════════════════════════════════════════════════════════════
def page_guide():
    st.markdown('<div class="sec-head">🚀 How to Run & Host</div>', unsafe_allow_html=True)
    ...
    with tab1:
        st.subheader("Run on Your PC — 4 Steps")
        for i, (title, desc) in enumerate([
            ("Install Python 3.10+",
             "Download from python.org — tick <b>Add to PATH</b>"),
            ("Download project files",
             "Put <code>app.py</code>, <code>requirements.txt</code>, <code>.streamlit/config.toml</code> in one folder"),
            ("Install dependencies",
             "<code>pip install -r requirements.txt</code>"),
            ("Launch dashboard",
             "<code>streamlit run app.py</code> → opens at <b>http://localhost:8501</b>"),
        ], 1):
            st.markdown(f'<div class="step-box"><b>Step {i}: {title}</b><br>{desc}</div>',
                        unsafe_allow_html=True)
        st.code("""
...
└── .streamlit/
    ...
    with tab2:
        st.success("☁️ Free global link — anyone can open your app!")
        for i, (title, desc) in enumerate([
            ("Create GitHub account","github.com → Sign up free"),
            ("New public repository","+ → New repo → <code>employee-attrition-ml</code> → Public"),
            ("Upload files","Drag <code>app.py</code>, <code>requirements.txt</code>, <code>.streamlit/config.toml</code>"),
            ("Go to Streamlit Cloud","share.streamlit.io → Sign in with GitHub"),
            ("Deploy","New app → Select repo → Main file: <code>app.py</code> → Deploy!"),
            ("Share","<code>https://yourname-employee-attrition-ml.streamlit.app</code>"),
        ], 1):
            st.markdown(f'<div class="step-box"><b>Step {i}: {title}</b><br>{desc}</div>',
                        unsafe_allow_html=True)
    ...
    with tab3:
        for i, (title, desc) in enumerate([
            ("Create account","huggingface.co → Sign up free"),
            ("New Space","Profile → New Space → SDK: <b>Streamlit</b>"),
            ("Upload files","Upload <code>app.py</code> + <code>requirements.txt</code>"),
            ("Share","<code>https://huggingface.co/spaces/yourname/attrition-ml</code>"),
        ], 1):
            st.markdown(f'<div class="step-box"><b>Step {i}: {title}</b><br>{desc}</div>',
                        unsafe_allow_html=True)

    with tab4:
        st.subheader("🎓 Internship Submission Checklist")
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("### 📁 Files to Submit")
            for icon, fname, desc in [
                ("✅","app.py","Streamlit dashboard"),
                ("✅","employee_attrition_prediction.ipynb","Jupyter Notebook"),
                ("✅","requirements.txt","Dependencies"),
                ("✅","README.md","Documentation"),
                ("📋","Project_Report.pdf","3–5 page report"),
                ("📊","Presentation.pptx","8–10 slides"),
                ("🔗","GitHub repo link","Public URL"),
                ("🌐","Live app link","Streamlit Cloud URL"),
                ("📸","Screenshots","5–6 dashboard images"),
                ("🎥","demo_video.mp4","2–3 min screen recording"),
            ]:
                st.markdown(f"{icon} **{fname}** — {desc}")

        with col2:
            st.markdown("### 📊 PPT Slides (8–10)")
            for s in ["Slide 1: Title + Name + RISE Internship + Date",
                      "Slide 2: Problem Statement",
                      "Slide 3: HR Dataset & Features",
                      "Slide 4: Tools & Technologies",
                      "Slide 5: ML Pipeline Diagram",
                      "Slide 6: EDA — Top Attrition Factors Charts",
                      "Slide 7: Key Attrition Drivers (Feature Importance)",
                      "Slide 8: Model Results + ROC Curve",
                      "Slide 9: At-Risk Employee Screenshot",
                      "Slide 10: What I Learned + Live App Link"]:
                st.markdown(f"• {s}")

        st.markdown("---")
        st.markdown("""<div class="insight-box">
        <b>🎤 Presentation Tips for Project 4:</b><br>
        • Show the <b>live dashboard prediction</b> for a sample employee<br>
        • Show the <b>Feature Importance chart</b> — explain the top 3 attrition drivers<br>
        • Display the <b>At-Risk Employee List</b> — this is the key business deliverable<br>
        • Explain <b>why Recall > Accuracy</b> for attrition (missing a leaver is costly)<br>
        • Mention real-world connection: <b>IBM HR Analytics</b> is a famous dataset for this exact problem<br>
        • Share your <b>GitHub link and live Streamlit link</b> on the last slide
        </div>""", unsafe_allow_html=True)
